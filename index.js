...
function handleEvent(event) {
  switch (event.type) {
    ...
    case 'join':
      return replyText(event.replyToken, `สวัสดีปีใหม่ jWun-Bot 02 บอทเจ๊มาแล้วจ้าา ลองพิมพ์ jWun help ดูจิ`);
    case 'leave':
...
const replyStyle = (token, texts) => {
  if (texts.indexOf('ด่า') !== -1 && texts.indexOf('jWun') !== -1 && texts.indexOf('ให้หน่อย') !== -1) {
  ...
  } else if (texts.indexOf('jWun') !== -1 && texts.length > 4) {
